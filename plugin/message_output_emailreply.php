 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class message_output_emailreply extends message_output
{
    /**
    ...
    function send_message($eventdata)
    {
        ...
        if (isset($eventdata->replyto)) {
            ...
            $replyto = isset($USER) ? $USER->email : '';
            $replytoname = isset($USER) ? $USER->firstname : '';
            $replyto = $replyto;
    ...
    function config_form($preferences)
    {
        ...
        if (!empty($CFG->messagingallowemailoverride)) {
            $inputattributes = array(
                'size' => '30', 'name' => 'email_email', 'value' => $preferences->email_email,
                'id' => 'email_email'
            );
